...
void Color_Print(const TCHAR* text)
{
...
void Color_Printf(const TCHAR* format, ...)
{
    va_list arglist;
...
void Print_At(int column, int row, const TCHAR* format, ...)
{
    va_list arglist;
...
void Color_Print_At(int column, int row, const TCHAR* format, ...)
{
    va_list arglist;
    int was_bold;
...
void Normal_Print_At(int column, int row, const TCHAR* format, ...)
{
    va_list arglist;
...
void BlinkPrintAt(int column, int row, const TCHAR* format, ...)
{
    va_list arglist;
...
void Exit_Screen(void)
{
    if (flags.partitions_have_changed == TRUE)
    {
        ...
        Input(0, 0, 0, ESC, 0, 0, ESCE, 0, 0, _T('\0'), _T('\0'));
...
void Warn_Incompatible_Ext(void)
{
    ...
    Input(0, 0, 0, ESC, 0, 0, ESCR, 0, 0, _T('\0'), _T('\0'));
}
...
void Interactive_User_Interface(void)
{
    ...
    con_set_textcolor(flags.screen_color & 0x0F);
    ...
    do
    {
        ...
        if ((menu == CPDP) || (menu == CEDP))
        {
            ...
            if (counter > 3)
            {
                ...
                Color_Printf(_T(" %d"), (flags.drive_number - 127));
                ...
                Input(0, 0, 0, ESC, 0, 0, ESCC, 0, 0, _T('\0'), _T('\0'));
                menu = MM;
            }
        }
        ...
        {
            /*         Create_BootEasy_MBR();
            Color_Print_At( 4, 22, _T("BootEasy MBR has been created.") );
            Input( 0, 0, 0, ESC, 0, 0, ESCC, 0, 0, _T('\0'), _T('\0') );*/
        }
        ...
        if (menu == AMBR)
        {
            ...
            FILE* file_pointer = _tfopen(home_path, _T("rb"));
...
int Standard_Menu(int menu)
{
    ...
    const TCHAR* option_1 = _T("");
    const TCHAR* option_2 = _T("");
    const TCHAR* option_3 = _T("");
    const TCHAR* option_4 = _T("");

    TCHAR optional_char_1 = _T('\0');
    TCHAR optional_char_2 = _T('\0');
    ...
    for (;;)
    {
        ...
        {
            Print_Centered(0, _T(FD_NAME) _T(" V") _T(VERSION), STANDARD);
...
void Ask_User_About_FAT32_Support(void)
{
    ...
    con_print(_T("  "));
    flags.fat32 = (int)Input(1, -1, -1, YN, 0, 0, NONE, 1, 0, _T('\0'), _T('\0'));
}
...
int Inform_About_Trimmed_Disk(void)
{
    ...
    con_print(_T(" "));
    return (int)Input(1, -1, -1, YN, 0, 0, NONE, 1, 0, _T('\0'), _T('\0'));
}
...
void Change_Current_Fixed_Disk_Drive(void)
{
    ...
    Display_All_Drives(0, svarlang_str(3, 6), BOLD);

    con_set_cursor_xy(5, con_get_cursor_y()+1);
    con_printf(svarlang_str(10, 190), (flags.maximum_drive_number - 127));
    int new_drive_number = (int)Input(1, -1, -1, NUM, 1, (flags.maximum_drive_number - 127),
                                      ESCR, (flags.drive_number - 127), 0, _T('\0'), _T('\0'));
...
int Create_DOS_Partition_Interface(int type)
{
    ...
    long maximum_partition_size_in_MB = (long) Max_Pri_Part_Size_In_MB(type);
    ...
    if (type == PRIMARY)
    {
        ...
        flags.esc = FALSE;
        input = Input(1, -1, -1, YN, 0, 0, ESCR, 1, 0, _T('\0'), _T('\0'));
        if (flags.esc == TRUE)
        {
            return (1);
        }
        ...
        if (input == 1)
        {
            ...
            if (flags.fprmt == TRUE && (input >= 128) && (input <= 2048))
            {
                ...
                con_print(_T(" "));
                flags.fat32 = !Input(1, -1, -1, YN, 0, 0, NONE, 1, 0, _T('\0'), _T('\0'));
            }
            ...
            int partition_slot_just_used = Create_Primary_Partition(numeric_type, input);
            if ((flags.drive_number - 128) == 0)
            {
                Set_Active_Partition_If_None_Is_Active(partition_slot_just_used);
            }
    ...
    if (partition_created == FALSE)
    {
        ...
        unsigned long maximum_possible_percentage = Convert_To_Percentage(maximum_partition_size_in_MB, pDrive->disk_size_mb);

        con_printf(_T(" (\33[1m%d%%\33[22m)"), maximum_possible_percentage);
            ...
            (flags.version == 6))
        {
            input = Input(4, -1, -1, NUMP, 1, maximum_partition_size_in_MB,
                          ...
                          maximum_possible_percentage, _T('\0'), _T('\0'));
        }
        else
        {
...
int Create_Logical_Drive_Interface(void)
{
    ...
    if ((pDrive->ptr_ext_part->start_head != 0 || pDrive->ptr_ext_part->start_sect != 1) &&
        flags.align_4k == FALSE && allow_unaligned_ext == FALSE)
    {
        Clear_Screen(0);

        Color_Print_At(35, 3, svarlang_str(250, 5));    // NLS:WARNING!

        con_set_cursor_xy(5, 8);
        con_print(svarlang_str(30, 10));        // NLS:FDISK is currently in cylinder alignment mode, but the\n    extended partition does not start on a cylinder boundary!\n\n
        con_print(svarlang_str(30, 11));        // NLS:    While unlikely, this MAY result in compatibility problems.\n\n
        con_print(svarlang_str(30, 12));        // NLS:If your system depends on proper cylinder alignment you should\n    consider re-creating the extended partition.

        Print_At(4, 21, svarlang_str(30, 13));  // NLS:Create logical drive in non-aligned extended partition...?
        con_print(_T(" "));
    ...
    if (pDrive->ext_free_space >= 2)
    {
        do
        {
            ...
            unsigned long maximum_partition_size_in_MB = Max_Log_Part_Size_In_MB();
            ...
            int y_base = con_get_cursor_y() + 2;

            if (drive_created == TRUE)
            {
                /* drive created message */
                Color_Print_At(4, y_base + 3, svarlang_str(10, 70));        // NLS:Logical DOS Drive created, drive letters changed or added
            }

            if (_T('Z') == Determine_Drive_Letters())
            {
                // Достигли предела логических дисков
                con_set_cursor_xy(5, y_base + 3);   // 23
                con_clreol();

                Color_Print_At(4, y_base + 2, svarlang_str(10, 71));        // NLS:Maximum number of Logical DOS Drives installed
                Input(0, 0, -1, ESC, 0, 0, ESCC, 0, 0, _T('\0'), _T('\0'));
                if (flags.fprmt == TRUE)
                {
                    flags.fat32 = FALSE;
                }
            ...
            con_set_cursor_xy(5, y_base - 2);   // 17   // ?????? fix to 18
            con_clreol();

            /* print extended partition size */
            Print_At(4, y_base - 3, svarlang_str(10, 6), pDrive->ext_size_mb);              // 17

            /* print maximum partition size */
            Print_At(4, y_base - 2, svarlang_str(10, 63), maximum_partition_size_in_MB);    // 18

            int maximum_possible_percentage = (int)Convert_To_Percentage(maximum_partition_size_in_MB, pDrive->ext_size_mb);

            con_printf(_T(" (\33[1m%d%%\33[22m)"), maximum_possible_percentage);

            /* enter partition size */
            Print_At(4, y_base, svarlang_str(10, 72));      // NLS:Enter logical drive size in Mbytes or percent (%%) of\n    extended partition size...:
            con_print(_T(" "));
            ...
            {
                input = Input(4, -1, -1, NUMP, 1, maximum_partition_size_in_MB, ESCR, maximum_partition_size_in_MB,
                              maximum_possible_percentage, _T('\0'), _T('\0'));
            }
            else
            {
                input = Input(7, -1, -1, NUMP, 1, maximum_partition_size_in_MB, ESCR, maximum_partition_size_in_MB,
                              maximum_possible_percentage, _T('\0'), _T('\0'));
            }
            ...
            {
                /* switch th FAT-16? */
                Print_At(4, y_base + 1, svarlang_str(10, 62));      // NLS:This drive is a FAT32 by default, switch to FAT16 (Y/N)?
                con_print(_T(" "));
                flags.fat32 =
                    !Input(1, -1, -1, YN, 0, 0, NONE, 1, 0, _T('\0'), _T('\0'));
            }
    ...
    Display_Extended_Partition_Information_SS();
    int y_base = con_get_cursor_y() + 2;
    Color_Print_At(4, y_base + 2, svarlang_str(10, 73));    // NLS:All available space in the Extended DOS Partition\n    is assigned to logical drives.
    Input(0, 0, -1, ESC, 0, 0, ESCC, 0, 0, _T('\0'), _T('\0'));
...
void Delete_Extended_DOS_Partition_Interface(void)
{
    ...
    flags.esc = FALSE;
    input = (int)Input(1, -1, -1, YN, 0, 0, ESCR, 0, 0, _T('\0'), _T('\0'));
...
int Delete_Logical_Drive_Interface(void)
{
    ...
    int input_ok = FALSE;
    int y_base = con_get_cursor_y() + 4;

    do
    {
        con_set_cursor_xy(5, y_base);
        ...
        if ((flags.del_non_dos_log_drives == TRUE) && (pDrive->num_of_non_dos_log_drives > 0))
        {
            ...
            input = (int)Input(1, -1, -1, RCHAR, 67, 90, ESCR, 0, 0, _T('1'), pDrive->num_of_non_dos_log_drives + _T('0'));

        } else {
            input = (int)Input(1, -1, -1, RCHAR, 67, 90, ESCR, 0, 0, _T('\0'), _T('\0'));
        }
        ...
        if (input_ok == FALSE)
        {
            ...
            Color_Print_At(4, y_base + 2, svarlang_str(10, 250));
        }
    }
    ...
    Normal_Print_At(4, y_base + 2, svarlang_str(10, 200));
    con_print(_T(" "));
    flags.esc = FALSE;
    input = (int)Input(1, -1, -1, YN, 0, 0, ESCR, 0, 0, _T('\0'), _T('\0'));
    ...
    {
        int error_code = Delete_Logical_Drive(drive_to_delete);
        Determine_Drive_Letters();
        ...
        y_base = con_get_cursor_y() + 4;
        if (!error_code)
        {
            Color_Print_At(4, y_base, svarlang_str(10, 31));    // NLS:Logical drive deleted
        }
        else
        {
            Color_Print_At(4, y_base, svarlang_str(10, 32));    // NLS:Error deleting logical drive!
        }
        input = (int)Input(0, 0, -1, ESC, 0, 0, ESCC, 0, 0, _T('\0'), _T('\0'));
    }
...
void Delete_N_DOS_Partition_Interface(void)
{
    ...
    con_print(_T("  "));
    input =
        (int)Input(1, -1, -1, NUM, 1, 4, ESCR, -1, 0, _T('\0'),
                   _T('\0')); /* 4 needs changed to the max num of partitions */
    ...
    if (flags.esc == FALSE)
    {
        ...
        {
            int error_code = Delete_Primary_Partition(input - 1);
...
void Delete_Primary_DOS_Partition_Interface(void)
{
    ...
    if (flags.esc == FALSE)
    {
        int partition_to_delete = input - 1;
        ...
        if (Is_Dos_Part(p->num_type))
        {
            ...
            if ((input == TRUE) && (flags.esc == FALSE))
            {
                if (input)
                {
                    ...
                    int error_code = Delete_Primary_Partition(partition_to_delete);
                    Display_Primary_Partition_Information_SS();
...
void Display_Extended_Partition_Information_SS(void)
{
    ...
    Partition* p;
    int print_index = 4;
    int last_line = (int) con_get_height() - 6 - 1;
        ...
        (brief_partition_table[(flags.drive_number - 128)][5] > 0))
    {
        ...
        Print_At(column_index, 3, svarlang_str(10, 12));
        ...
        do
        {
            ...
            if (brief_partition_table[(flags.drive_number - 128)][index] > 0)
            {
                if (IsRecognizedFatPartition(brief_partition_table[(flags.drive_number - 128)][index]))
                {
                    ...
                    Color_Print_At(column_index + 0, print_index, _T("%c"),
                                   drive_lettering_buffer[(flags.drive_number - 128)][index]);
                    Color_Print_At(column_index + 1, print_index, _T(":"));

                    /* Display volume label */
                    Print_At(column_index + 4, print_index, _T("%11s"), A2T(p->vol_label));

                } else {
                    if (flags.del_non_dos_log_drives == TRUE)
                    {
                        /* Display drive number */
                        Color_Print_At(column_index + 0, print_index, _T("%c"), drive_letter_or_questionmark(
                                           drive_lettering_buffer[(flags.drive_number - 128)][index]));
                    }
                }
                ...
                Print_At(column_index + 25, print_index, _T("%-9s"), part_type_descr_short(p->num_type));

                /* Display usage in % */
                unsigned long usage = Convert_To_Percentage(p->num_sect, pDrive->ext_num_sect);

                Print_At(column_index + 35, print_index, _T("%3lu%%"), usage);
                print_index++;
            }
            index++;
        }
    ...
    con_set_cursor_xy(5, print_index+2);    // 18);
    con_clreol();
...
Beginning:
    ...
    if (flags.extended_options_flag == FALSE)
    {
        Print_Centered(1, svarlang_str(10, 83), BOLD);  // NLS:Display Logical DOS Drive Information
    }
    else
    {
        Print_Centered(1, svarlang_str(3, 5), BOLD);    // NLS:Display / Modify partition information
    }

    Display_Extended_Partition_Information_SS();        // Вывод информации по лог. дискам в разделе EXT-DOS
    ...
    if (flags.extended_options_flag == FALSE)
    {
        ...
        int input_ok = FALSE;       // fix: add initialize
        int input;
        Print_At(4, con_get_cursor_y(), svarlang_str(10, 84));      // NLS:Enter the character of the logical drive you want to modify...?
        con_print(_T(" "));
        ...
        do
        {
            ...
            if ((flags.del_non_dos_log_drives == TRUE) && (pDrive->num_of_non_dos_log_drives > 0))
            {
                ...
                input = (int)Input(1, -1, -1, RCHAR, 68, 90, ESCC, 0, 0, _T('1'), pDrive->num_of_non_dos_log_drives + _T('0'));

            } else {
                input = (int)Input(1, -1, -1, RCHAR, 68, 90, ESCC, 0, 0, _T('\0'), _T('\0'));
            }
...
Beginning:
    ...
    {
        Print_At(4, 17, svarlang_str(10, 80));      // NLS:Do you want to display the logical drive information (Y/N)......?
        con_print(_T(" "));
        con_save_cursor_xy();
        if (flags.extended_options_flag == TRUE)
        {
            Print_At(4, 20, svarlang_str(10, 81));  // NLS:Optional:  Type the number of the partition to modify.

            con_restore_cursor_xy();
            input = (int)Input(1, -1, -1, YN, 0, 0, ESCR, 1, 0, _T('1'), _T('4'));

            if (((input - 48) >= 1) && ((input - 48) <= 4))
            {
                Modify_Primary_Partition_Information(input - 48);   // Переходим в меню изменения раздела (тип, скрытие, активный статус)
                goto Beginning;
            }
        }
        else
        {
            input = (int)Input(1, -1, -1, YN, 0, 0, ESCR, 1, 0, _T('\0'), _T('\0'));
        }

        if (input == TRUE)
        {
            Display_Or_Modify_Logical_Drive_Information();  // Вывод инфы по логическим дискам (и их редактирование)
            if (flags.extended_options_flag == TRUE)
            {
                goto Beginning;
            }
    ...
    else
    {
        ...
        {
            Print_At(4, 18, svarlang_str(10, 82)); // NLS:Enter the number of the partition you want to modify (1-4)...?
            con_print(_T(" "));

            flags.esc = FALSE;
            input = (int)Input(1, -1, -1, NUM, 1, 4, ESCR, 1, 0, _T('\0'), _T('\0'));
        ...
        if (flags.extended_options_flag == FALSE)
        {
            ...
            for (index = 0; index < 4; index++)
            {
                ...
                if (p->num_type > 0)
                {
                    ...
                    {
                        Print_At(5, (cursor_offset + 9), _T("%c:"),
                                 drive_letter_or_questionmark(
                                     drive_lettering_buffer[(flags.drive_number - 128)][index]));
                    }

                    /* Partition Number */
                    Color_Print_At(8, (cursor_offset + 9), _T("%d"), (index + 1));
                    ...
                    /* Volume Label */
                    Print_At(32, (cursor_offset + 9), _T("%-11s   "), p->vol_label);

                    /* Mbytes */
                    // TODO: GPT: возможно стоит печатать реальный размер?
                    if (p->num_type == 0xee || p->num_type == 0xeb)
                    {
                        // данные о размере всё равно не верны.
                        Print_UL(0);
                    } else
                    {
                        Print_UL(p->size_in_MB);
                    ...
                    /* System */
                    Print_At(55, (cursor_offset + 9), _T("%-15s  "), part_type_descr(p->num_type));
        ...
        else
        {
            ...
            for (index = 0; index < 4; index++)
            {
                ...
                if (p->num_type > 0)
                {
                    ...
                    /* Partition Type */
                    Print_At(10, (cursor_offset + 9), _T("%3d"), p->num_type);

                    /* Status */
                    if (p->active_status > 0)
                    {
                        Print_At(19, (cursor_offset + 9), _T("A"));
                    }
                    ...
                    /* Description */
                    Print_At(33, (cursor_offset + 9), _T("%-15s"),
                    ...
                    Print_At(51, (cursor_offset + 9), _T("%3d%%"), usage);

                    /* Starting Cylinder */
                    Print_At(60, (cursor_offset + 9), _T("%6lu"), p->start_cyl);

                    /* Ending Cylinder */
                    Print_At(69, (cursor_offset + 9), _T("%6lu"), p->end_cyl);
...
void List_Partition_Types(void)
{
    ...
    do
    {
        ...
        Color_Print_At(column, row, _T("%3d "), index);
        con_printf(_T("%s"), part_type_descr(index));
...
void Modify_Extended_Partition_Information(int logical_drive_number)
{
    ...
    do
    {
        ...
        if (IsRecognizedFatPartition(p->num_type))
        {
            Color_Print_At(5, 9, _T("%c:"),
                drive_lettering_buffer[(flags.drive_number - 128)][(logical_drive_number + 4)]);
        }

        /* Partition Number */
        Print_At(8, 9, _T("%d"), (logical_drive_number + 1));

        /* Partition Type */
        Print_At(10, 9, _T("%3d"), (p->num_type));
        ...
        /* Description */
        Print_At(33, 9, _T("%-15s"), part_type_descr(p->num_type));

        /* Usage */
        usage = Convert_To_Percentage(p->size_in_MB, pDrive->ext_size_mb);

        Print_At(51, 9, _T("%3d%%"), usage);

        /* Starting Cylinder */
        Print_At(60, 9, _T("%6lu"), p->start_cyl);

        /* Ending Cylinder */
        Print_At(69, 9, _T("%6lu"), p->end_cyl);

        /* NLS:"Choose one of the following: "*/
        Print_At(4, 12, svarlang_str(9, 2));

        Color_Print_At(4, 14, _T("1."));
        con_print(svarlang_str(10, 102));
        Color_Print_At(4, 15, _T("2."));
        con_print(svarlang_str(10, 90));
        Color_Print_At(4, 16, _T("3."));
        con_print(svarlang_str(10, 104));
        /*
      Color_Print_At(44,15,_T("4."));
  ...
  */
        ...
        flags.esc = FALSE;
        int input = (int)Input(1, -1, -1, NUM, 1, 3, ESCC, -1, 0, _T('\0'), _T('\0'));
        if (flags.esc == TRUE)
        {
            input = 99;
            finished = TRUE;
        ...
        if (input == 1)
        {
            ...
            con_print(_T(" "));
            flags.esc = FALSE;
            input = (int)Input(3, -1, -1, NUM, 1, 255, ESCC, -1, 0, _T('\0'), _T('\0'));
            if (flags.esc == FALSE)
            {
                p->num_type = input;
...
void Modify_Primary_Partition_Information(int partition_number)
{
    ...
    do
    {
        ...
        /* Partition Number */
        Color_Print_At(8, 9, _T("%d"), (partition_number + 1));
        ...
        /* Usage */
        unsigned long usage = Convert_To_Percentage(p->size_in_MB, pDrive->disk_size_mb);
        ...
        flags.esc = FALSE;
        int input = (int)Input(1, -1, -1, NUM, 1, 4, ESCC, -1, 0, _T('\0'), _T('\0'));
        if (flags.esc == TRUE)
        {
            input = 99;
            finished = TRUE;
        ...
        if (input == 1)
        {
            ...
            flags.esc = FALSE;
            input =
                (int)Input(3, -1, -1, NUM, 1, 255, ESCC, -1, 0, _T('\0'), _T('\0'));
            if (flags.esc == FALSE)
            {
...
int Set_Active_Partition_Interface(void)
{
    ...
    for (index = 0; index <= 3; index++)
    {
        ...
        if (IsRecognizedFatPartition(p->num_type))
        {
            ...
            if ((available_partition_counter == 1) && (p->active_status == 0x80))
            {
                first_available_partition_active = TRUE;
            }
            partition_settable[index] = TRUE;
        }
        else if ((p->num_type > 0) && (flags.set_any_pri_part_active == TRUE))
        {
            available_partition_counter++;
            if ((available_partition_counter == 1) && (p->active_status == 0x80))
            {
    ...
    if ((available_partition_counter == 1) && (first_available_partition_active == TRUE))
    {
