...
static int svl_add_str(struct svl_lang* l, unsigned short id, const char* s)
{
    ...
    if ((l->strings_cap < svl_strings_bytes(l) + len) || (l->dict_cap < (l->num_strings + 1) * sizeof(struct
        dict_entry)))
    {
        return (0);
    }
...
static int svl_write_lang(const struct svl_lang* l, FILE* fd, int compflag, unsigned short* buffrequired)
{
    ...
    if (compflag)
    {
        ...
        {
            printf(
                "lang %c%c mvcomp-ressed (%u bytes -> %u bytes) mvcomp stream at most %u bytes ahead of raw data (%u bytes needed for in-place decomp)\n",
