...
int Create_Logical_Drive(int numeric_type, unsigned long size_in_MB) {
    ...
    // TODO: Сомнительная фича, малость подкорректировал
    if ((max_sz_cyl - req_sz_cyl) <= req_sz_cyl || size_in_MB == max_sz_mb) {
        req_sz_cyl = max_sz_cyl;
    }
    ...
    Partition* p = &pDrive->log_drive[free_space_loc];
    ...
    if (free_space_loc == 0) {
        ...
        nep->rel_sect = chs_to_lba(pDrive, nep->start_cyl, nep->start_head, nep->start_sect) - ep->rel_sect;
        nep->num_sect = chs_to_lba(
            pDrive, nep->end_cyl, nep->end_head, nep->end_sect) -
            chs_to_lba(pDrive, nep->start_cyl, nep->start_head, nep->start_sect) +1;
    }

    /* calculate start sector from beginning of extended partition */
    unsigned long ext_start_sect = chs_to_lba(pDrive, nep->start_cyl, nep->start_head, nep->start_sect);
    unsigned long start_sect = ext_start_sect + pDrive->total_sect;
    if (flags.align_4k) {
        start_sect = align_up(start_sect);
    }
    unsigned long end_sect = chs_to_lba(pDrive, end_cyl, pDrive->total_head, pDrive->total_sect);

    /* Compute the size of the partition. */
    unsigned long part_sz = end_sect - start_sect + 1;

    lba_to_chs(start_sect, pDrive, &p->start_cyl, &p->start_head, &p->start_sect);
...
unsigned long Max_Pri_Free_Space_In_MB(void) {
    ...
    unsigned long max_size_mb = Convert_Cyl_To_MB((pDrive->pri_free_space), pDrive->total_head + 1, pDrive->total_sect);
...
unsigned long Max_Log_Free_Space_In_MB(void) {
    ...
    unsigned long max_size_mb = Convert_Cyl_To_MB((pDrive->ext_free_space), pDrive->total_head + 1, pDrive->total_sect);
...
unsigned long Max_Log_Part_Size_In_MB(void) {
    ...
    unsigned long stored_max_size_mb = max_size_mb;
...
static unsigned long Number_Of_Cylinders(unsigned long size) {
    ...
    unsigned long num_head = size / pDrive->total_sect;
    if ((size % pDrive->total_sect) != 0) {
        num_head++;
    }

    unsigned long num_cyl = num_head / (pDrive->total_head + 1);
...
TCHAR drive_letter_or_questionmark(TCHAR drive_letter) {
    if (drive_letter) {
        return drive_letter;
    } else {
        return _T('?');
